...
public class FabricF3ServerNameManager extends F3ServerNameManager {
	...
	public void sendF3ServerName(F3ServerName f3ServerName, ChatPluginServerPlayer player) {
		...
		player.sendPacket(VersionUtils.getVersion().isAtLeast(Version.V1_20_2)
				? NewNetworking.getBrandPacket(PlaceholderManager.getInstance().translatePlaceholders(f3ServerName.getText(player.getLanguage(), true), player, placeholderTypes) + "§r")
				: MixinPlugin.getBrandPacket(PlaceholderManager.getInstance().translatePlaceholders(f3ServerName.getText(player.getLanguage(), true), player, placeholderTypes) + "§r")
				);
	}
	
	private static class NewNetworking {
		
		public static CustomPayloadS2CPacket getBrandPacket(String brand) {
			return new CustomPayloadS2CPacket(new BrandCustomPayload(brand));
		}
