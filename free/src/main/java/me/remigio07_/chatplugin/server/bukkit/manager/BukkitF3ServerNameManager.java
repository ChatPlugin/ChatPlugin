...
import me.remigio07_.chatplugin.api.server.event.f3servername.F3ServerNameSendEvent;
import me.remigio07_.chatplugin.api.server.f3servername.F3ServerName;
...
import me.remigio07_.chatplugin.common.f3servername.F3ServerNamePacket;
import me.remigio07_.chatplugin.server.f3servername.BaseF3ServerNameManager;
	...
	public void sendF3ServerName(F3ServerName f3ServerName, ChatPluginServerPlayer player) {
		...
		if (ProxyManager.getInstance().isEnabled())
			ProxyManager.getInstance().sendPluginMessage(
					new PacketSerializer("F3ServerName")
					.writeServerID()
					.writeUTF(player.getUUID().toString())
					.writeUTF(PlaceholderManager.getInstance().translatePlaceholders(f3ServerName.getText(player.getLanguage(), true), player, placeholderTypes))
					);
