...
package me.remigio07_.chatplugin.server.util;
...
import me.remigio07_.chatplugin.server.sponge.SpongeReflection;

public class Utils extends me.remigio07_.chatplugin.api.server.util.Utils {
	...
	public static void setTitle(ChatPluginServerPlayer viewer, String title, int rows) {
		if (Environment.isBukkit()) {
			...
			else packet = BukkitReflection.getInstance("PacketPlayOutOpenWindow", new Class[] { int.class, BukkitReflection.getLoadedClass("Containers"), BukkitReflection.getLoadedClass("IChatBaseComponent") }, windowID, VersionUtils.getVersion().isAtLeast(Version.V1_14) ? BukkitReflection.getField("Containers", null, "GENERIC_9X" + rows, ATTEMPTS[rows - 1]) : ("minecraft:generic_9x" + rows), component);
